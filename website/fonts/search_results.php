...
$dropdown = '';
$ratingList = '';
if(isset($_GET['search'])||($_GET['search']&&$_GET['tsort']))
{
	if(isset($_GET['search']))
	{
		$searchq = $_GET['search'];
		$searchq = preg_replace("#[^0-9a-z]#i", "", $searchq);
		$query = pg_query($dbconn, "SELECT * FROM \"Tour\" NATURAL JOIN \"Location\" NATURAL JOIN \"isCategory\" NATURAL JOIN \"Tour Category\" WHERE lower(\"tour_Name\") LIKE lower('%$searchq%') OR lower(\"Category_Name\") LIKE lower('%$searchq%') OR lower(\"City\") LIKE lower('%$searchq%') OR lower(\"State-Province\") LIKE lower('%$searchq%')");
	}
