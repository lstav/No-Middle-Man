...
if($_SESSION['tgid'])
{
		...
		if(!empty($result)) {
			if(pg_num_rows($result) > 0) {
				...
				$desc = trim($row['g_desc']);
				$license = trim($row['g_License']);
				$company = trim($row['Company']);
				$phone = trim($row['g_telephone']);
			}
		}
		...
		if(!empty($_POST['new-uemail'])||!empty($_POST['new-ufname'])||!empty($_POST['new-ulname'])
			...
			||!empty($_POST['new-addr'])||!empty($_POST['new-telephone'])||!empty($_POST['new-desc'])||!empty($_POST['new-desc'])
			||!empty($_POST['new-company'])||!empty($_POST['new-license'])||!empty($_POST['new-desc'])||!empty($_FILES['image']['name']))
		{
			...
			if(!empty($_POST['new-telephone']))
			{
				...
				$query = pg_query($dbconn, "UPDATE \"Tour Guide\" SET \"g_telephone\" = '$newutelephone' 
				WHERE \"g_Email\" = '$uemail'");
				$telephone = $_SESSION['telephone'] = $newutelephone;
			}
			if(!empty($_POST['new-desc']))
			{
				$newdesc = test_input(strip_tags($_POST["new-desc"]));
				$query = pg_query($dbconn, "UPDATE \"Tour Guide\" SET \"g_desc\" = '$newdesc' 
				WHERE \"g_Email\" = '$uemail'");
				$desc = $_SESSION['tgdesc'] = $newdesc; //$telephone = $newutelephone;
			}
			...
			{
				$newcompany = test_input(strip_tags($_POST["new-company"]));
				$query = pg_query($dbconn, "UPDATE \"Tour Guide\" SET \"Company\" = '$newcompany' 
				WHERE \"g_Email\" = '$uemail'");
				$company = $_SESSION['tgcompany'] = $newcompany;
			}
			if(!empty($_POST['new-license']))
			{
				$newlicense = test_input(strip_tags($_POST["new-license"]));
				$query = pg_query($dbconn, "UPDATE \"Tour Guide\" SET \"g_License\" = '$license' 
				WHERE \"g_Email\" = '$uemail'");
				$license = $newlicense;
			}
			...
			if(!empty($_POST['new-upass']) && !empty($_POST['old-upass']) && !empty($_POST['con-new-upass']))
			{
				...
				$checkpass = trim($_POST['new-upass']);
				...
				if($oldupass == $upass)
				{
					if($newupass == $connewupass) {
						if(strlen($checkpass) >= 8) {
							...
							$errorMsg ="Password length must be higher than 8";
						}
					} else {
            ...
            <label class="control-label" for="inputLast">Company
              Description: </label>
            <textarea maxlength="300" id="inputLast" class="form-control" rows="5" name = "new-desc" placeholder="'.$desc.'" type="text"></textarea>
          </div>
		  					   <div class="control-group">
									<label class="control-label" for="inputEmail">Company Name:</label>
		
									<div class="controls">
										<input id="inputEmail" name = "new-company" placeholder="'.$company.'" type="text">
									</div>
								</div>
								
								<div class="control-group">
									<label class="control-label" for="inputEmail">License</label>
		
									<div class="controls">
										<input id="inputEmail" name = "new-license" placeholder="'.$license.'" type="text">
									</div>
								</div>
  ...
  <div role="tabpanel"> 
    ...
    <div class="tab-content">
      <div role="tabpanel" class="tab-pane active" id="profile">
        <div class="area">
          <form class="form-horizontal" method = "post" enctype ="multipart/form-data" action="guide_account.php">
